 ...
 * @param hintColor       Use this color for hinttext (don't need to use hintClass)    [mode:field|text]
 * @param hintClass       Classname to use as hint (w/o Text in)                       [mode:field|text|class]
	...
	var ztinputhintmehtods = {
		...
		init: function(options) {
			...
			return this.each(function() {
				...
				if (!container.is('input:text, input:password, textarea')) {
					return false;
				}
				...
				// ### Class-Mode
				// #########################################
				else if (ztinputhintOptions.mode == 'text') {
					...
					container.blur(function() {
						...
						// if empty
						if (container.val() == '' || container.val() == textHint) {
							
							if (container.val() == textHint) container.val('');
							
							// set color if hintClass is empty and hintColor not empty
							if (!ztinputhintOptions.hintClass && ztinputhintOptions.hintColor) {
								container.data('defaultColor', container.css('color'));
								container.css('color', ztinputhintOptions.hintColor);
							}
							
							// set class if hintClass is not empty and field didn't have it
							else if (ztinputhintOptions.hintClass && !container.hasClass(classHint)) {
								container.addClass(ztinputhintOptions.hintClass);
							}
							
							container.val(textHint);
					...
					container.focus(function() {
						...
						if (!ztinputhintOptions.hintClass && ztinputhintOptions.hintColor && container.data('defaultColor')) container.css('color', container.data('defaultColor'));
						else {
							// remove hintClass
		...
		/**
		 * Just on Fields-Mode to re-set hinttext
		 * @param string
		 */
